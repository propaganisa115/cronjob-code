...
for key in datas:
    urldata_user = "https://" + str(key['sekolah_domain']) + "/api/main/data_user"
    if len(str(key['sekolah_domain'])) < 0:
        ...
        try:
            ...
            if (respClient.status_code == 200):
                ...
                data_user.append({'jumlah_siswa_aktif': siswa,'updated_at': today})


                print(data_user)
                urlSeon = "https://api.seonindonesia.net/sekolah/update/" + key['sekolah_domain']
                ...
                def tambahData(key):
                    TambahData = {"jumlah_siswa_aktif": key['jumlah_siswa_aktif'],"updated_at": key['updated_at']}
                    tambahdatas = str(TambahData).replace("'", '"')
                    try:
                        resp = requests.patch(urlSeon, headers=headersPython, data=tambahdatas)
                ...
                with ThreadPoolExecutor(max_workers=None) as exec:
                    try:
                        ...
                        log_scheduler_global = {"domain": key['sekolah_domain'],
                                                "kode_scheduler": "scraping update jumlah siswa aktif", "record_time": today,
            ...
            error = str(e).replace("'", "")
            log_scheduler_global = {"domain": str(key['sekolah_domain']), "kode_scheduler": "scraping update jumlah siswa aktif",
