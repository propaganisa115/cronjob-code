...
for key in datas:
    ...
    if len(str(key['sekolah_domain'])) < 0:
        ...
        try:
            ...
            if (respClient.status_code == 200):
                ...
                with ThreadPoolExecutor(max_workers=None) as exec:
                    try:
                        ...
                        log_scheduler_global = {"domain": domain,
                ...
                log_scheduler_global = {"domain": str(domain),
                                        "kode_scheduler": "scraping total active storage per school data", "record_time": today,
                                        "status": "failed",
            ...
            error = str(e).replace("'", "")
            log_scheduler_global = {"domain": str(domain), "kode_scheduler": "scraping total active storage per school data",
