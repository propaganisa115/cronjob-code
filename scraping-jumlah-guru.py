...
for key in datas:
    urlJumlahGuru = "https://" + str(key['sekolah_domain']) + "/api/main/data_user"
    if len(str(key['sekolah_domain'])) < 0:
        log_scheduler_global = {"domain": "empty", "kode_scheduler": "scraping number of teachers per school data", "record_time": today,
        ...
        try:
            respClient = requests.get(urlJumlahGuru, headers=headersClient)
            ...
            if (respClient.status_code == 200):
                ...
                data_jumlahsiswa.append({'jumlahguru': guru, 'sekolah_domain': str(domain),'record_time': today})



                urlSeon = "https://api.seonindonesia.net/jumlah_guru/create"
                ...
                def tambahData(key):
                    TambahData = {"jumlahguru": key['jumlahguru'], "sekolah_domain": key['sekolah_domain'], "record_time": key['record_time']}
                ...
                with ThreadPoolExecutor(max_workers=None) as exec:
                    try:
                        ...
                        log_scheduler_global = {"domain": key['sekolah_domain'],
                                                "kode_scheduler": "scraping number of teachers per school data", "record_time": today,
            ...
            error = str(e).replace("'", "")
            log_scheduler_global = {"domain": str(key['sekolah_domain']), "kode_scheduler": "scraping number of teachers per school data",
