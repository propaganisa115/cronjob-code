...
for key in datas:
    ...
    if len(str(key['sekolah_domain']))<0:
         log_scheduler_global = {"domain":"empty","kode_scheduler":"scraping seonpoint data","record_time": today ,"status":"failed","error_message":"sekolah_domain is empty with id school "+str(key['id'])+", you can check in the table sekolah"}
        ...
        try:
            ...
            if (respClient.status_code == 200):
                ...
                def ambilData(key):
                    data_seon_point.append({'id': key['id'], 'id_user': key['id_user'], 'type_user': key['type_user'], 'action': key['action'],'point': key['point'], 'keterangan': key['keterangan'], 'record_time': key['record_time']})
                ...
                with ThreadPoolExecutor(max_workers=None) as exec:
                    try:
                        ...
                        log_scheduler_global = {"domain":key['sekolah_domain'],"kode_scheduler":"scraping seonpoint data","record_time": today ,"status":"success","error_message":""}
                        log_scheduler_global =str(log_scheduler_global).replace("'", '"')
                        resp = requests.post(urlLogSchedulerGlobal, headers=headersPython2, data=log_scheduler_global)
                    except Exception as e:
                        error=str(e).replace("'", "")
                        log_scheduler_global = {"domain":key['sekolah_domain'] ,"kode_scheduler": "scraping seonpoint data" ,"record_time": today ,"status":"sucess","error_message":error[:200] }
                        log_scheduler_global =str(log_scheduler).replace("'", '"')
                        resp = requests.post(urlLogSchedulerGlobal, headers=headersPython2, data=log_scheduler_global)
            else:
                log_scheduler_global = {"domain":str(key['sekolah_domain']),"kode_scheduler":"scraping seonpoint data","record_time": today ,"status":"failed","error_message":"response return http error with status code "+str(respClient.status_code)  }
                log_scheduler_global =str(log_scheduler_global).replace("'", '"')
                resp = requests.post(urlLogSchedulerGlobal, headers=headersPython2, data=log_scheduler_global)
        except Exception as e:
            error=str(e).replace("'", "")
            log_scheduler_global = {"domain":str(key['sekolah_domain']),"kode_scheduler":"scraping seonpoint data","record_time": today ,"status":"failed","error_message":error[:200]}
            log_scheduler_global =str(log_scheduler_global).replace("'", '"')
            resp = requests.post(urlLogSchedulerGlobal, headers=headersPython2, data=log_scheduler_global)
